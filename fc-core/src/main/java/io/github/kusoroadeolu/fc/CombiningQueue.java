...
class CombiningQueue<T> extends CombiningCollection<T> implements Queue<T> {
    private final Combiner<Queue<T>> combiner;
    private final WaitStrategy waitStrategy;

    public CombiningQueue() {
        this(new FlatCombiner<>(new ArrayDeque<>()), WaitStrategy.park(1));
    }

    public CombiningQueue(Combiner<Queue<T>> combiner, WaitStrategy strategy) {
